 ...
 * pool library. This header file contains the entire library, including the
 * main BS::thread_pool class and the helper classes BS::multi_future,
 * BS::blocks, BS:synced_stream, and BS::timer.
 */
 ...
 * parallelize_loop() and push_loop().
 *
 * @tparam T1 The type of the first index in the range. Should be a signed or
 * unsigned integer.
 * @tparam T2 The type of the index after the last index in the range. Should be
 * a signed or unsigned integer. If T1 is not the same as T2, a common type will
...
public:
   ...
   */
  blocks(const T1 first_index_, const T2 index_after_last_,
...
public:
   ...
   *
   * @param thread_count_ The number of threads to use. The default value is the
   ...
   * submitting each block separately to the queue. Returns a multi_future
   * object that contains the futures for all of the blocks.
   *
   ...
   * @return A multi_future object that can be used to wait for all the blocks
   * to finish. If the loop function returns a value, the multi_future object
  ...
  parallelize_loop(const T1 first_index, const T2 index_after_last, F &&loop,
   ...
   * @brief Parallelize a loop by automatically splitting it into blocks and
   * submitting each block separately to the queue. Returns a multi_future
   * object that contains the futures for all of the blocks. This overload is
   * used for the special case where the first index is 0.
   *
   * @tparam F The type of the function to loop through.
   ...
   * words, it will be equivalent to "for (T i = 0; i < index_after_last; ++i)".
   * Note that if index_after_last == 0, no blocks will be submitted.
   * @param loop The function to loop through. Will be called once per block.
   * Should take exactly two arguments: the first index in the block and the
   * index after the last index in the block. loop(start, end) should typically
   * involve a loop of the form "for (T i = start; i < end; ++i)".
  ...
  template <typename F, typename T,
            typename R = std::invoke_result_t<std::decay_t<F>, T, T>>
  [[nodiscard]] multi_future<R> parallelize_loop(const T index_after_last,
                                                 F &&loop,
                                                 const size_t num_blocks = 0) {
    return parallelize_loop(0, index_after_last, std::forward<F>(loop),
                            num_blocks);
  }
  ...
  /**
   * @brief Parallelize a loop by automatically splitting it into blocks and
   * submitting each block separately to the queue. Does not return a
   * multi_future, so the user must use wait_for_tasks() or some other method to
  ...
  template <typename F, typename T1, typename T2,
            typename T = std::common_type_t<T1, T2>>
  void push_loop(const T1 first_index, const T2 index_after_last, F &&loop,
                 const size_t num_blocks = 0) {
    blocks blks(first_index, index_after_last,
                num_blocks ? num_blocks : thread_count);
    if (blks.get_total_size() > 0) {
   ...
   */
  template <typename F, typename T>
  void push_loop(const T index_after_last, F &&loop,
                 const size_t num_blocks = 0) {
   ...
   * wait forever). Note: To wait for just one specific task, use submit()
...
private:
   ...
   *
   * @param thread_count_ The parameter passed to the constructor or reset(). If
   ...
   */
  [[nodiscard]] concurrency_t
  determine_thread_count(const concurrency_t thread_count_) {
    if (thread_count_ > 0)
  ...
  /**
   * @brief A condition variable used to notify wait_for_tasks() that a tasks is
   * done.
   */
...
public:
   ...
   *
   * @param out_stream_ The output stream to print to. The default value is
   * std::cout.
   */
   ...
   * simultaneously, as long as they all exclusively use the same synced_stream
   * object to print.
   *
   * @tparam T The types of the items
   * @param items The items to print.
  ...
  /**
   * @brief A stream manipulator to pass to a synced_stream (an explicit cast of
   * std::endl). Prints a newline character to the stream, and then flushes it.
  ...
  /**
   * @brief A stream manipulator to pass to a synced_stream (an explicit cast of
   * std::flush). Used to flush the stream.
   */
